# arcgis-economic-distress-index

Creating an economic distress index layer in ArcGIS using Census data with Python.
...
3 - Explore - If you are more into Python, a good place to start is `jupyter lab` from the root of the project, and look in the `./notebooks` directory. If GIS is more your schtick, open the project `./arcgis/arcgis-economic-distress-index.aprx`.
...
- `make env` - builds the Conda environment with all the name and dependencies from `environment_dev.yml` and installs the local project package `arcgis_economic_distress_index` using the command `python -m pip install -e ./src/src/arcgis_economic_distress_index` so you can easily test against the package as you are developing it.
